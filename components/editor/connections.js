...
function toggleConn(otherId) {
  ...
  if (n.connections.includes(otherId)) {
    ...
    // blocks: compute B's dates from its blockers and cascade
    if (pendingConnType === 'blocks') {
      if (typeof computeNodeDates === 'function') computeNodeDates(other);
      other.updated = new Date().toISOString();
      if (typeof propagateBlockedDatesData === 'function') propagateBlockedDatesData(other.id);
      if (typeof recalcMetrics === 'function') recalcMetrics();
      if (S.currentId === other.id) renderEditor();
    }
  }
